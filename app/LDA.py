...
def plot_data(train_, trn_targets, title, labels, new_data):
    ...
    print("New Data Dim:", len(new_data[0]))
    ...
    if len(new_data[0]) > 2:
        ...
        for i in range(len(train_)):
            ...
            # Plot the new data
            ax.scatter3D(new_data[0][0], new_data[0][1], new_data[0][2], color="purple", edgecolors="none", label="New Data", alpha=1, marker="X", s=100)

    elif len(new_data[0]) == 2:
        ax = plt.axes()
        
        for i in range(len(train_)):
            if trn_targets[i] == 0:
                ax.scatter(train_[i][0], train_[i][1], color=color[0], edgecolors="none", label=labels[0], alpha=0.5)
            elif trn_targets[i] == 1:
                ax.scatter(train_[i][0], train_[i][1], color=color[1], edgecolors="none", label=labels[1], alpha=0.5)
            elif trn_targets[i] == 2:
                ax.scatter(train_[i][0], train_[i][1], color=color[2], edgecolors="none", label=labels[2], alpha=0.5)
            elif trn_targets[i] == 3:
                ax.scatter(train_[i][0], train_[i][1], color=color[3], edgecolors="none", label=labels[3], alpha=0.5)
            elif trn_targets[i] == 4:
                ax.scatter(train_[i][0], train_[i][1], color=color[4], edgecolors="none", label=labels[4], alpha=0.5)
            elif trn_targets[i] == 5:
                ax.scatter(train_[i][0], train_[i][1], color=color[5], edgecolors="none", label=labels[5], alpha=0.5)

        # Plot the new data
        ax.scatter(new_data[0][0], new_data[0][1], color="purple", edgecolors="none", label="New Data", alpha=1, marker="X", s=100)

    elif len(new_data[0]) == 1:
        ax = plt.axes()

        for i in range(len(train_)):
            if trn_targets[i] == 0:
                ax.scatter(train_[i][0], 1, color=color[0], edgecolors="none", label=labels[0], alpha=0.5)
            elif trn_targets[i] == 1:
                ax.scatter(train_[i][0], 1, color=color[1], edgecolors="none", label=labels[1], alpha=0.5)
            elif trn_targets[i] == 2:
                ax.scatter(train_[i][0], 1, color=color[2], edgecolors="none", label=labels[2], alpha=0.5)
            elif trn_targets[i] == 3:
                ax.scatter(train_[i][0], 1, color=color[3], edgecolors="none", label=labels[3], alpha=0.5)
            elif trn_targets[i] == 4:
                ax.scatter(train_[i][0], 1, color=color[4], edgecolors="none", label=labels[4], alpha=0.5)
            elif trn_targets[i] == 5:
                ax.scatter(train_[i][0], 1, color=color[5], edgecolors="none", label=labels[5], alpha=0.5)

            # Plot the new data
            ax.scatter(new_data[0][0], 1, color="purple", edgecolors="none", label="New Data", alpha=1, marker="X", s=100)
